...
//        this.sequelize = new Sequelize(database, username, password, options || {});
        ...
        this.sequelize = new Sequelize("lexixi_bird", "root", "123456", {
            dialect: 'mysql',
            define: {
                underscored: true,
    ...
    AutoSequelize.prototype.run = function (options, callback) {
        ...
        this.sequelize.query("SHOW TABLES;", null, {raw: true})
            .error(function (err) {
                console.log('ERR: ' + err);
            })
            .success(function (tables) {
                ...
                async.each(tables, function (table, _callback) {
                    ...
                    async.each(tableNames, function (table, _callback) {
                        ...
                        text[table] += spaces + "return sequelize.define('" + _s.classify(table) + "', { \n";

                        fields.forEach(function (field, i) {
                            text[table] += spaces + spaces + _s.camelize(field) + ": {\n";
                            var fieldAttr = Object.keys(_tables[table][field]);
                            // Serial key for postgres...
                            var defaultVal = _tables[table][field].defaultValue;
                            console.log(_tables[table][field])
                        ...
                        "{"+"\n"
                            ...
                            + spaces + spaces+"underscoredAll:true,"+"\n"
                            + spaces + spaces+"timestamps:false"+"\n"
                            + "}"+
                            ");\n};\n";
                        _callback(null);
                    }, function () {
    ...
    AutoSequelize.prototype.write = function (attributes, callback) {
        ...
        async.series([
            ...
            async.each(tables, function (table, _callback) {
                fs.writeFile(self.options.directory + '/' + _s.classify(table) + '.js', attributes[table], function (err) {
